...
class Shop extends MY_Shop_Controller
{
    ...
    {
        if (!$this->loggedIn) {
            $this->bpas->send_json(['status' => 'error', 'message' => lang('please_login')]);
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (count($user_addresses) >= 6) {
                $this->bpas->send_json(['status' => 'error', 'message' => lang('already_have_max_addresses'), 'level' => 'error']);
            }
            ...
            if ($id) {
                ...
                $this->bpas->send_json(['redirect' => $_SERVER['HTTP_REFERER']]);
            } else {
            ...
            $this->bpas->send_json(['status' => 'error', 'message' => validation_errors()]);
        } else {
            ...
            if ($guest_checkout || $address = $this->shop_model->getAddressByID($this->input->post('address'))) {
                ...
                foreach ($this->cart->contents() as $item) {
                    ...
                    if ($product_details = $this->shop_model->getProductForCart($item['product_id'])) {
                        $price = $this->bpas->setCustomerGroupPrice(($product_details->special_price ? $product_details->special_price : $product_details->price), $this->customer_group);
                        $price = $this->bpas->isPromo($product_details) ? $product_details->promo_price : $price;
                        if ($item['option']) {
                ...
                $grand_total = $this->bpas->formatDecimal(($total + $total_tax + $shipping), 4);
                ...
                // $this->bpas->print_arrays($data, $products, $customer, $address);
    ...
    public function order_received($id = null, $hash = null)
    {
        if ($inv = $this->shop_model->getOrder(['id' => $id, 'hash' => $hash])) {
            ...
            try {
                if ($this->bpas->send_email(($user ? $user->email : $customer->email), $subject, $message, null, null, $attachment, $cc, $bcc)) {
    ...
    public function orders($id = null, $hash = null, $pdf = null, $buffer_save = null)
    {
        ...
        if ($id && !$pdf) {
            ...
            if ($buffer_save) {
                return $this->bpas->generate_pdf($html, $name, $buffer_save, $this->data['biller']->invoice_footer);
            }
    ...
    public function search()
    {
        ...
        if ($products = $this->shop_model->getProducts($filters)) {
            ...
            foreach ($products as &$value) {
                $value['details'] = character_limiter($this->bpas->decode_html(strip_tags($value['details'])), 140);
                ...
                if ($this->shop_settings->hide_price) {
                    ...
                    $value['price']                  = $this->bpas->setCustomerGroupPrice($value['price'], $this->customer_group);
                    $value['formated_price']         = $this->bpas->convertMoney($value['price']);
                    $value['promo_price']            = $this->bpas->isPromo($value) ? $value['promo_price'] : 0;
                    $value['formated_promo_price']   = $this->bpas->convertMoney($value['promo_price']);
                    $value['special_price']          = isset($value['special_price']) && !empty($value['special_price']) ? $this->bpas->setCustomerGroupPrice($value['special_price'], $this->customer_group) : 0;
                    $value['formated_special_price'] = $this->bpas->convertMoney($value['special_price']);
                }
            }
            ...
            $this->bpas->send_json(['filters' => $filters, 'products' => $products, 'pagination' => $pagination, 'info' => $info]);
        } else {
            $this->bpas->send_json(['filters' => $filters, 'products' => false, 'pagination' => false, 'info' => false]);
        }
    }
    ...
    public function send_message()
    {
        ...
        if ($this->form_validation->run() == true) {
            try {
                if ($this->bpas->send_email($this->shop_settings->email, $this->input->post('subject'), $this->input->post('message'), $this->input->post('email'), $this->input->post('name'))) {
                    $this->bpas->send_json(['status' => 'Success', 'message' => lang('message_sent')]);
                }
                $this->bpas->send_json(['status' => 'error', 'message' => lang('action_failed')]);
            } catch (Exception $e) {
                $this->bpas->send_json(['status' => 'error', 'message' => $e->getMessage()]);
            }
        } elseif ($this->input->is_ajax_request()) {
            $this->bpas->send_json(['status' => 'Error!', 'message' => validation_errors(), 'level' => 'error']);
        } else {
    ...
    {
        $page = $this->shop_model->getPostBySlug($slug);
        if (!$page) {
            redirect('notify/error_404');
        }
        $this->data['sidebar_posts'] = $this->shop_model->getSidebarPosts();
        $this->data['page']       = $page;
        $this->data['page_title'] = $page->title;
        $this->data['page_desc']  = $page->description;
        $this->page_construct('posts/post', $this->data);
    }
    // Display Page
}
