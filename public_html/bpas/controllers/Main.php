...
class Main extends MY_Shop_Controller
{
    public function __construct()
    {
        ...
        $this->perPage = $this->Settings->rows_per_page;
    ...
    public function index()
    {
        ...
        $this->data['home_products']     = $this->shop_model->getHomeProducts($conditions);
        ...
        $this->data['home_posts']        = $this->shop_model->getAllPosts();

        $this->data['vendors']           = $this->site->getAllCompanies('biller');

        $this->page_construct('index', $this->data);
    }
    public function membership(){
   
        $this->data['getuser']    = 111;
        ...
        $this->page_construct('pages/menu', $this->data);
    }
    ...
    public function forgot_password()
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->bpas->send_json(validation_errors());
        } else {
            $identity = $this->ion_auth->where('email', strtolower($this->input->post('email')))->users()->row();
            if (empty($identity)) {
                $this->bpas->send_json(lang('forgot_password_email_not_found'));
            }

            $forgotten = $this->ion_auth->forgotten_password($identity->email);
            if ($forgotten) {
                $this->bpas->send_json(['status' => 'success', 'message' => $this->ion_auth->messages()]);
            } else {
                $this->bpas->send_json(['status' => 'error', 'message' => $this->ion_auth->errors()]);
            }
        }
    }
    ...
    function loadMoreProduct(){
        ...
        $conditions = array();
        // Get last post ID
        $lastID = $this->input->get('id');
        // Get post rows num
        $conditions['where'] = array('bpas_products.id <'=>$lastID);
        $conditions['returnType'] = 'count';
        $this->data['postNum'] = $this->shop_model->getHomeProducts($conditions);
        // Get posts data from the database
        $conditions['returnType'] = '';
        $conditions['order_by'] = "id DESC";
        $conditions['limit'] = $this->perPage;
        $this->data['more_products'] = $this->shop_model->getHomeProducts($conditions);
        $this->data['postLimit'] = $this->perPage;
        $this->load->view($this->theme.'load-more-products.php', $this->data);
    }
    ...
    public function login__($m = null)
    {
    ...
    public function reset_password($code = null)
    {
     ...
     public function pricing(){
        $this->data['page_title'] = lang('login');
        $this->data['page_desc']  = $this->shop_settings->description;
        $this->page_construct('pages/pricing', $this->data);  
    }
    public function vendors(){
        $this->data['page_title'] = lang('vendors');
        $this->data['page_desc']  = $this->shop_settings->description;
        $this->data['vendors']    = $this->site->getAllCompanies('biller');
        $this->page_construct('pages/vendors', $this->data);  
    }
    public function e_menu($category_slug = null, $subcategory_slug = null, $brand_slug = null, $promo = null)
    {
        ...
        $conditions['limit']    = $this->perPage;
        $this->data['home_products']     = $this->shop_model->getHomeProducts($conditions);
        $this->data['categories'] = $this->shop_model->getAllCategories();
