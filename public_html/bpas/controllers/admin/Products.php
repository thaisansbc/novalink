...
class Products extends MY_Controller
{
    ...
    public function add($id = null, $type = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data  = [
                ...
                'details'           => $this->bpas->clear_tags($this->input->post('details')),
                'product_details'   => $this->bpas->clear_tags($this->input->post('product_details')),
                'cf4'               => $this->bpas->clear_tags($this->input->post('cf4')),
                'cf5'               => $this->bpas->clear_tags($this->input->post('cf5')),
                'cf6'               => $this->bpas->clear_tags($this->input->post('cf6')),
    ...
    public function edit($id = null, $type=null)
    {
        ...
        $product_code = $this->input->post('code');
        $prod_code    = $item_code ? ($product_code."|".$item_code) : $product_code;
        if ($this->form_validation->run('products/edit_product') == true) {
    ...
    {
        $this->bpas->checkPermissions('cost_adjustments');
        $this->data['billers'] = $this->site->getBillers();
        $this->data['biller'] = $biller_id ? $this->site->getCompanyByID($biller_id) : null;
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('products'), 'page' => lang('inventory')), array('link' => '#', 'page' => lang('cost_adjustments')));
        $meta = array('page_title' => lang('cost_adjustments'), 'bc' => $bc);
        $this->page_construct('products/cost_adjustments', $meta, $this->data);
    }
    ...
    function getCostAdjustments($biller_id = NULL)
    {
        ...
        $this->datatables
            ->select("
                {$this->db->dbprefix('cost_adjustments')}.id as id, 
                {$this->db->dbprefix('cost_adjustments')}.date, 
                reference_no, CONCAT({$this->db->dbprefix('users')}.last_name, ' ', {$this->db->dbprefix('users')}.first_name) as created_by, note, cost_adjustments.attachment")
    ...
    {
        $this->bpas->checkPermissions('cost_adjustments', TRUE);
    ...
    {
        $this->bpas->checkPermissions('cost_adjustments-add',true);
        $this->load->helper('security');
        $this->form_validation->set_rules('userfile', lang("upload_file"), 'xss_clean');
        $this->form_validation->set_rules('biller', lang("biller"), 'required');
        if ($this->form_validation->run() == true) {
            $biller_id = $this->input->post('biller');
            $project_id = $this->input->post('project');
            if ($this->Owner || $this->Admin || $this->bpas->GP['products-date']) {
                $date = $this->bpas->fld($this->input->post('date'));
            } else {
                    ...
                    if($row['code']!=''){
                        if($product_details = $this->products_model->getProductByCode($row['code'])){
                            if($row['cost'] > 0){
                                ...
                                $warehouse_products = $this->products_model->getAllWarehouseByProducts($product_details->id); 
                                ...
                                if($this->Settings->accounting == 1 && $product_details->quantity > 0){ 
                                    ...
                                    $accTrans[] = array(
                                        'transaction' => 'CostAdjustment',
                                        'transaction_date' => $date,
                                        'account' => $productAcc->adjustment_account,
                                        'amount' => -$amount,
                                        'narrative' => 'Product Code: '.$product_details->code.'#'.'Qty: '.$product_details->quantity.'#'.'Cost: '.$new_cost,
                                        'description' => $note,
                                        'biller_id' => $biller_id,
            ...
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("products/cost_adjustments/");
        }
    ...
    function add_cost_adjustment()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $warehouse_products = $this->products_model->getAllWarehouseByProducts($product_id); 
                $unit = $this->site->getProductUnit($product_id,$product_details->unit);
                if($warehouse_products){
                    foreach($warehouse_products as $warehouse_product){
                        if($warehouse_product->quantity > 0 || $warehouse_product->quantity < 0){
                ...
                if($this->Settings->accounting == 1 && ($product_details->quantity > 0 || $product_details->quantity < 0)){ 
                    ...
                    $accTrans[] = array(
                        ...
                        'tran_date'     => $date,
                        'reference_no'  => $reference_no,
                        'account_code'  => $productAcc->adjustment_account,
                        'amount'        => $amount * (-1),
                        'narrative'     => 'Product Code: '.$product_details->code.'#'.'Qty: '.$product_details->quantity.'#'.'Cost: '.$new_cost,
                        'description'   => $note,
                        'biller_id'     => $biller_id,
        ...
        if ($this->form_validation->run() == true && $this->products_model->addCostAdjustment($data, $products, $accTrans, $stockmoves)) {
            ...
            $this->data['billers'] = $this->site->getAllCompanies('biller');
            $this->data['projects']            = $this->site->getAllProject();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('products'), 'page' => lang('inventory')), array('link' => admin_url('products/cost_adjustments'), 'page' => lang('cost_adjustments')), array('link' => '#', 'page' => lang('add_cost_adjustment')));
            $meta = array('page_title' => lang('add_cost_adjustment'), 'bc' => $bc);
            $this->page_construct('products/add_cost_adjustment', $meta, $this->data);
