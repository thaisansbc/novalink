...
class system_settings extends MY_Controller
{
    ...
    public function modules()
    {
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateSetting($data)) {
            ...
            $bc                            = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('system_settings')]];
            $meta                          = ['page_title' => lang('system_settings'), 'bc' => $bc,'page_view'=>'full'];
   ...
   function promotion()
     ...
     public function ranks()
    {
        ...
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('brands')]];
        $meta                = ['page_title' => lang('ranks'), 'bc' => $bc];
        $this->page_construct('settings/ranks', $meta, $this->data);
    }
    ...
    public function levels()
    {
        ...
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('brands')]];
        $meta                = ['page_title' => lang('ranks'), 'bc' => $bc];
        $this->page_construct('settings/levels', $meta, $this->data);
    }
    ...
    public function matching_bonus()
    {
        ...
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('brands')]];
        $meta                = ['page_title' => lang('ranks'), 'bc' => $bc];
        $this->page_construct('settings/matching_bonus', $meta, $this->data);
    }
    public function getMatchingBonus()
    {
        ...
        $this->datatables
            ->select("
                {$this->db->dbprefix('matching_bonus')}.id,
                ranks.name as rame,
                CONCAT({$this->db->dbprefix('matching_bonus')}.left_bv,'/',{$this->db->dbprefix('matching_bonus')}.right_bv) as downline_bv,
                CONCAT({$this->db->dbprefix('matching_bonus')}.enroll_left,' Left/ ',{$this->db->dbprefix('matching_bonus')}.enroll_right,' Right') as personal_enroll,
                level1,
                level2,
                level3,
                level4,
            ...
            ")
            ->from('matching_bonus')
            ->join('ranks', 'ranks.id=matching_bonus.rank_id', 'left')
