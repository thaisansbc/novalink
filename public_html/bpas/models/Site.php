...
class Site extends CI_Model
{
 ...
 public function getMainSuggestions($term, $limit = 10,$main=null)
    {
       $this->db->select("
            {$this->db->dbprefix('users')}.id,
            CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) AS text,
            CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) AS value,
            {$this->db->dbprefix('users')}.phone
        ", false);
       
        $this->db->where(" 
                (
                    {$this->db->dbprefix('users')}.id LIKE '%" . $term . "%' OR 
                    {$this->db->dbprefix('users')}.first_name LIKE '%" . $term . "%' OR 
                    {$this->db->dbprefix('users')}.last_name LIKE '%" . $term . "%' OR 
                    {$this->db->dbprefix('users')}.company LIKE '%" . $term . "%' OR 
                    {$this->db->dbprefix('users')}.phone LIKE '%" . $term . "%' 
                ) 
            ");
        // $this->db->where('users.right_node','');
        if ($main) {
            $this->db->where('bpas_users.id !=', $main);
        }

        $this->db->where('(bpas_users.right_node = 0 OR bpas_users.left_node = 0)');
           $this->db->where("(bpas_users.node = 'main' OR bpas_users.node = 'sub')");
        //   var_dump($main);
...
/**
     * Get user details with basic info
     */
    ...
    /**
     * Get all matching bonus configurations
     */
    public function getMatchingBonuses()
    {
        $this->db->select('m.*, r.name as rank_name');
    ...
    /**
     * Get user's left and right nodes
     */
     ...
     * Update user nodes
     */
    public function updateUserNodes($user_id, $left_node = null, $right_node = null)
    {
        $data = [];
        if ($left_node !== null) {
  ...
  public function getschedulesByID($id) 
    ...
    public function update_bv_qty($total, $member)
    {
        ...
        if ($saleman != null && !empty($this->Settings->each_sale) && !$this->Customer && $total >= $this->Settings->each_sale) {
            ...
            $this->db->update('users', ['bonus' => $total_points], ['id' => $member]);
        }
        return true;
    }
    public function getBonusBySalemanID($id)
    {
        $this->db->select('sum(bonus) as total_bonus');
        $q = $this->db->get_where('sponsor_member', ['sponsor_id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
