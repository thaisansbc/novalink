...
class Shop_model extends CI_Model
{
    ...
    public function getFeaturedProducts($limit = 16, $promo = true)
    {
        ...
        $this->db->order_by('cf2','ASC');
        return $this->db->get('products')->result();
    }
    public function getHomeProducts($params = array()){
        ...
        if(array_key_exists("products.id",$params)){
    ...
    {
        return $this->db->get_where('posts', ['slug' => $slug], 1)->row();
    }
    public function getSidebarPosts($limit = 16)
    {
        $this->db->select()
        ->where('active', 1)
        ->limit($limit);
