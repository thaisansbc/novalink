...
class Auth_model extends CI_Model
{
    ...
    public function getMainNode() {
        if (!$this->Owner && $this->session->userdata('user_id')) {
            $this->db->where('id',$this->session->userdata('user_id'));
        }else{
            $this->db->where('node','main');
        }
    ...
    public function buildTree($id = 0) {
        ...
        // Build left and right child recursively
        if (!empty($node->left_node) && $node->left_node != 0) {
            $left = $this->buildTree($node->left_node);
            if ($left) $data['children'][] = $left;
        }

        if (!empty($node->right_node) && $node->right_node != 0) {
            $right = $this->buildTree($node->right_node);
            if ($right) $data['children'][] = $right;
        }
    ...
    public function login($identity, $password, $remember = false)
    {
        ...
        $query                 = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, view_right, edit_right, allow_discount, show_cost, show_price,emp_id as employee_id')
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === true) {
                ...
                $ldata = ['user_id' => $user->id, 'ip_address' => $this->input->ip_address(), 'login' => $identity];
                ...
                $this->site->syncHRData();
                //$this->site->sendTelegram("login",true,true);
                return true;
            }
        }
    ...
    public function login_remembered_user()
    {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id,emp_id as employee_id, username, email, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, view_right, allow_discount, edit_right, show_cost, show_price')
    ...
    public function getBillerByID($id)
    {
        ...
        $q = $this->db->get_where('users', array('multi_biller' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function update($id, array $data, $upgs = [])
    {
        ...
        if(isset($data['main_node'])){
                ...
                $checkNodeleft = $this->checkNodeleft($id)->row();
                $checkNoderight = $this->checkNoderight($id)->row();

                

                if($checkNodeleft){
                    // $this->db->update($this->tables['users'], ['left_node' => 0 ], ['id' => $checkNodeleft->left_node]);
                    $this->db->update('users', ['left_node' => '0'], ['id' => $checkNodeleft->id]);
                }
                if($checkNoderight){
                    // $this->db->update($this->tables['users'], ['right_node' => 0 ], ['id' => $checkNoderight->right_node]);
                    $this->db->update('users', ['right_node' => '0'], ['id' => $checkNoderight->id]);
                }


                $infoMain = $this->user($data['main_node'])->row();
                if($infoMain->left_node==0){
                    $mainNote = $data['main_node'];
                    $sideNode = "left";
                ...
                if($sideNode){
                    $this->AssignSubnode($mainNote,$sideNode,$id);
                }
    ...
    {
        if ($this->db->update($this->tables['users'], ['signature' => $signature], ['id' => $id])) {
            $this->set_message('signature_updated');
            return true;
        }
        return false;
    }
    ...
    public function AssignSubnode($id,$nodeSide,$SubNoteID)
    {
        if($nodeSide){
            if($nodeSide=='right'){
                $this->db->update('users', ['right_node' => $SubNoteID], ['id' => $id]);
            }else{
                $this->db->update('users', ['left_node' => $SubNoteID], ['id' => $id]);
            }
            return true;
        } else {
            return false;
    ...
    public function checkNodeleft($id = null)
    {
        $this->trigger_events('user');
        ...
        $this->limit(1);
        $this->where($this->tables['users'] . '.left_node', $id);
    ...
    public function checkNoderight($id = null)
    {
        $this->trigger_events('user');
        ...
        $this->limit(1);
        $this->where($this->tables['users'] . '.right_node', $id);
    ...
    {
        $this->db->where('saleman_by', $id)->from('sales');
        return $this->db->count_all_results();
    }
    public function clear_award_points($id)
    {
        if ($this->db->update($this->tables['users'], ['award_points' => 0], ['id' => $id])) {
            $this->set_message('award_points_clear_successfully');
            return true;
        }
        return false;
    }
    ...
    public function getUserCommissionByID_M_Y($id, $mth, $year)
    {
        if ($this->site->getSettings()->cost_sale_commission){
            // $q = " SUM((bpas_FSI.total_profit) * bpas_users.commission / 100) AS commission ";
            $s_cost_before = " (
                SELECT 
            ...
            $q = "
                COALESCE(
                    SUM(
                        IF(bpas_sales.sale_status != 'returned',
                            IF(
                                COALESCE(
                                    (SELECT 
                                        COALESCE(SUM(bpas_payments.amount), 0) 
                                    FROM bpas_payments 
                                    WHERE bpas_payments.sale_id = bpas_sales.id AND (YEAR(bpas_payments.date) < YEAR(bpas_FSJ.date) OR YEAR(bpas_payments.date) <= YEAR(bpas_FSJ.date) AND MONTH(bpas_payments.date) < MONTH(bpas_FSJ.date))
                                ), 0) - bpas_FSI.total_cost < 0,
                                IF(
                                    COALESCE(
                                        (SELECT 
                                            COALESCE(SUM(bpas_payments.amount), 0) + bpas_FSJ.total_paid 
                                        FROM bpas_payments 
                                        WHERE bpas_payments.sale_id = bpas_sales.id AND (YEAR(bpas_payments.date) < YEAR(bpas_FSJ.date) OR YEAR(bpas_payments.date) <= YEAR(bpas_FSJ.date) AND MONTH(bpas_payments.date) < MONTH(bpas_FSJ.date))
                                    ), 0) - bpas_FSI.total_cost < 0,
                                    0,
                                    (COALESCE(
                                        ...
                                        FROM bpas_payments 
                                        WHERE bpas_payments.sale_id = bpas_sales.id AND (YEAR(bpas_payments.date) < YEAR(bpas_FSJ.date) OR YEAR(bpas_payments.date) <= YEAR(bpas_FSJ.date) AND MONTH(bpas_payments.date) < MONTH(bpas_FSJ.date))
                                    ), 0) - bpas_FSI.total_cost) * bpas_users.commission / 100
                                ),
                                bpas_FSJ.total_paid * bpas_users.commission / 100
                            ),
                            IF(
            ...
            // $q = " SUM((grand_total * commission) / 100) AS commission ";
            $q = " SUM((bpas_FSJ.total_paid * bpas_users.commission) / 100) AS commission ";
        }
        ...
        $sj = "
                (SELECT 
                    ...
                    bpas_payments.date,
                    SUM(bpas_payments.amount) AS total_paid
                FROM bpas_payments
                GROUP BY bpas_payments.sale_id, MONTH(bpas_payments.date) + '-' + YEAR(bpas_payments.date)) AS bpas_FSJ
            ";

        $num_month = date('m', strtotime($mth));
        $this->db->select($q)
        ->from('sales')
        ->join($si,     'FSI.sale_id = sales.id',      'left')
        ->join($sj,     'FSJ.sale_id = sales.id',      'left')
        ->join('users', 'users.id = sales.saleman_by', 'left')
        ->where('MONTH(bpas_FSJ.date)', $num_month)
        ->where('YEAR(bpas_FSJ.date)', $year)
        ->where('sales.saleman_by', $id)
        ->group_by('MONTH(bpas_FSJ.date) + "-" + YEAR(bpas_FSJ.date), sales.saleman_by');
    ...
    public function getStaffPaySlipByID($id){
        $q = $this->db->get_where('staff_payslip', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    }
    public function getStaffPaySlipByID_M_Y($id, $month, $year){
        $q = $this->db->get_where('staff_payslip', array('staff_id' => $id, 'month' => $month, 'year' => $year), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    }
    public function getAllStaffPaySlipByID($id){
        $q = $this->db->get_where('staff_payslip', array('staff_id' => $id));
        if ($q->num_rows() > 0) {
    ...
    //-------------saleman---------
    public function addSaleman($data = false, $share_commissions = false){
        if($data && $this->db->insert('users',$data)){
            $salesman_id = $this->db->insert_id();
            if($share_commissions){
                foreach($share_commissions as $share_commission){
                    $share_commission["salesman_id"] = $salesman_id;
                    $this->db->insert("share_commissions",$share_commission);
                }
            }
            return true;
        }
    ...
    public function updateSaleman($id = false, $data = false, $share_commissions = false){
        if ($this->db->update('users', $data, array("id"=>$id))) {
            $this->db->delete("share_commissions",array("salesman_id"=>$id));
            if($share_commissions){
                $this->db->insert_batch("share_commissions",$share_commissions);
            }
            return true;
        }
        return false;
    ...
    public function deleteSaleman($id = false){
        if($id && $this->db->delete("users",array("id"=>$id))){
            $this->db->delete("leader_commissions",array("salesman_id"=>$id));
            $this->db->delete("share_commissions",array("salesman_id"=>$id));
            return true;
        }
        return false;
    }
    public function EmployeeRegister($username, $password, $email, $additional_data = [], $active = false)
    {
        $this->trigger_events('pre_register');
    ...
    public function remote_allow($id = null)
    {
        $user = $this->user($id)->row();
        $data = [
            'is_remote_allow' => ($user->is_remote_allow == 1)?0:1,
        ];
        ...
        $return = $this->db->affected_rows() == 1;
        if ($return) {
            $this->set_message('remote_allow_status_updated_successfully');
        } else {
            $this->set_error('remote_allow_failed');
        }
        return $return;
    }
    public function getSponsor($sponsor_id = null) 
    {   
        if($sponsor_id){
    ...
    public function getLeftRightMembers($main_id,$position=null)
    {
        $result = [];
        $this->db->select('left_node, right_node');
        ...
        if($position =='left'){
            if (!empty($main->left_node) && $main->left_node != 0) {
                $this->traverseLeft($main->left_node, $result);
            }
        } elseif($position =='right'){
            if (!empty($main->right_node) && $main->right_node != 0) {
                $this->traverseright($main->right_node, $result);
            }
        }else{      
            if (!empty($main->left_node) && $main->left_node != 0) {
                $this->traverseLeft($main->left_node, $result);
    ...
    private function traverseLeft($node_id, &$result)
    {
        $this->db->select('id, first_name, last_name, left_node');
        ...
        $result[] = $node;
        if (!empty($node->left_node) && $node->left_node != 0) {
            $this->traverseLeft($node->left_node, $result);
        }
    }
    private function traverseright($node_id, &$result)
    {
        $this->db->select('id, first_name, last_name, right_node');
        $this->db->where('id', $node_id);
        $query = $this->db->get('users');
        if ($query->num_rows() == 0) {
            return;
        ...
        $result[] = $node;
        if (!empty($node->right_node) && $node->right_node != 0) {
            $this->traverseright($node->right_node, $result);
        }
    }
    ...
    // new function for calculate  Rack  ==================================
    public function getSubMembers($main_id)
    {
        $this->db->select('left_node, right_node');
        $this->db->where('id', $main_id);
        $q = $this->db->get('users');
     ...
     public function getUserRank($user_id, $start_date = null, $end_date = null)
    {
        $total_bv = $this->getMemberBV($user_id, $start_date, $end_date);
        
        $left_bv = $this->getLegBV($user_id, 'left', $start_date, $end_date);
        $right_bv = $this->getLegBV($user_id, 'right', $start_date, $end_date);
        
        $left_enroll = $this->countLegEnrollments($user_id, 'left', $start_date, $end_date);
        $right_enroll = $this->countLegEnrollments($user_id, 'right', $start_date, $end_date);
        
        $this->db->select('m.*, r.name as rank_name, r.description');
        $this->db->from('matching_bonus m');
        $this->db->join('ranks r', 'r.id = m.rank_id');
        $this->db->order_by('m.rank_id', 'DESC');
        $query = $this->db->get();
        $ranks = $query->result();
        ...
        foreach ($ranks as $rank) {
            if ($left_bv >= $rank->left_bv && 
                $right_bv >= $rank->right_bv && 
                $left_enroll >= $rank->enroll_left && 
                $right_enroll >= $rank->enroll_right) {
                $user_rank = (object)[
                    'rank_id' => $rank->rank_id,
                    'rank_name' => $rank->rank_name,
                    'description' => $rank->description ?? '',
                    'left_bv' => $rank->left_bv,
                    'right_bv' => $rank->right_bv,
                    'enroll_left' => $rank->enroll_left,
                    'enroll_right' => $rank->enroll_right
                ];
                break;
            }
        }
     ...
     public function getMatchingBonusConfig($rank_id)
    {
        $this->db->select('*');
        $this->db->from('matching_bonus');
    ...
    public function getAllRankConfigs()
    {
        $this->db->select('m.*, r.name as rank_name, r.description');
        $this->db->from('matching_bonus m');
        $this->db->join('ranks r', 'r.id = m.rank_id');
        $this->db->order_by('m.rank_id', 'ASC');
        $query = $this->db->get();
        
        return $query->result();
    }
    private function getLegBV($user_id, $leg = 'left', $start_date = null, $end_date = null)
    {
        $this->db->select($leg . '_node as node_id');
        ...
        // Get BV for that leg's downline
        return $this->getMemberBV($user->node_id, $start_date, $end_date);
    }
    private function countLegEnrollments($user_id, $leg = 'left', $start_date = null, $end_date = null)
    {
        $this->db->select($leg . '_node as node_id');
        $this->db->where('id', $user_id);
        $user = $this->db->get('users')->row();
        ...
        return $this->countEnrollments($user->node_id, $start_date, $end_date);
    }
    public function checkQualificationStatus($user_id, $start_date = null, $end_date = null)
    {
        $this->db->select('left_node, right_node');
        $this->db->where('id', $user_id);
        $user = $this->db->get('users')->row();
        ...
        if ($user && $user->left_node) {
            $left_active = $this->checkMemberActive($user->left_node, $start_date, $end_date);
            $left_bv = $this->getMemberBV($user->left_node, $start_date, $end_date);
            $left_enroll = $this->countEnrollments($user->left_node, $start_date, $end_date);
        }
        
        if ($user && $user->right_node) {
            $right_active = $this->checkMemberActive($user->right_node, $start_date, $end_date);
            $right_bv = $this->getMemberBV($user->right_node, $start_date, $end_date);
            $right_enroll = $this->countEnrollments($user->right_node, $start_date, $end_date);
        }
        
        $pv = $this->getUserPV($user_id, $start_date, $end_date);
        $has_min_pv = $pv >= 50;
        
        $rank = $this->getUserRank($user_id, $start_date, $end_date);
        
        $bonus_config = $this->getMatchingBonusConfig($rank->rank_id);
        
        $meets_rank_requirements = false;
        if ($bonus_config) {
            $meets_rank_requirements = (
                $left_bv >= $bonus_config->left_bv && 
                $right_bv >= $bonus_config->right_bv &&
                $left_enroll >= $bonus_config->enroll_left &&
                $right_enroll >= $bonus_config->enroll_right
            );
        }
        
        return (object)[
            'is_fully_qualified' => ($left_active && $right_active && $has_min_pv),
            'has_two_legs' => ($left_active && $right_active),
            'has_min_pv' => $has_min_pv,
            'is_active' => $has_min_pv,
            'rank' => $rank->rank_name,
            'rank_id' => $rank->rank_id,
            'pv' => $pv,
            ...
            'right_enroll' => $right_enroll,
            'bonus_config' => $bonus_config,
            'meets_rank_requirements' => $meets_rank_requirements
        ];
    }
    public function checkMemberActive($user_id, $start_date = null, $end_date = null)
    {
        $pv = $this->getUserPV($user_id, $start_date, $end_date);
        return $pv >= 50;
    }
    public function getUserPV($user_id, $start_date = null, $end_date = null)
    {
        $this->db->select("SUM(max_serial) as total_pv");
        $this->db->from('sale_items');
        $this->db->where('saleman_by', $user_id);
    ...
    public function getMemberBV($user_id, $start_date = null, $end_date = null)
    {
        $this->db->select("SUM(max_serial) as total_bv");
        $this->db->from('sale_items');
        $this->db->where('saleman_by', $user_id);
    ...
    public function countEnrollments($user_id, $start_date = null, $end_date = null)
    {
        $this->db->select("COUNT(DISTINCT sale_id) as total_enrollments");
        $this->db->from('sale_items');
        $this->db->where('saleman_by', $user_id);
        ...
        $result = $this->db->get()->row();
        return $result->total_enrollments ?? 0;
    }
    public function getUserPackage($user_id)
    {
        $this->db->select('
            p.name, 
            p.price, 
            si.sponsor_bonus as commission_amount,
            si.sponsor_id,
            si.product_type,
            si.max_serial as bv
        ');
        ...
        $this->db->where('si.saleman_by', $user_id);
        $this->db->where('si.product_type', 'combo');
    ...
    public function calculateBinaryPerformance($user_id, $start_date = null, $end_date = null)
    {
        $qualification = $this->checkQualificationStatus($user_id, $start_date, $end_date);
        
        if (!$qualification->is_fully_qualified) {
            return (object)[
                'eligible' => false,
                'left_bv' => $qualification->left_bv,
                'right_bv' => $qualification->right_bv,
                'weak_leg_bv' => 0,
                'potential_bonus' => 0,
                'carry_forward' => abs($qualification->left_bv - $qualification->right_bv)
            ];
        }
        
        $weak_leg_bv = min($qualification->left_bv, $qualification->right_bv);
        $bonus_rate = 0.10; // 10% team bonus rate
        
        return (object)[
            'eligible' => true,
            'left_bv' => $qualification->left_bv,
            'right_bv' => $qualification->right_bv,
            'weak_leg_bv' => $weak_leg_bv,
            'potential_bonus' => $weak_leg_bv * $bonus_rate,
            'carry_forward' => abs($qualification->left_bv - $qualification->right_bv)
        ];
    }
    public function calculateLeadershipMatching($user_id, $downline_rank, $downline_bonus, $level = 1)
    {
        $user_rank = $this->getUserRank($user_id);
        $bonus_config = $this->getMatchingBonusConfig($user_rank->rank_id);
        ...
        switch($level) {
            case 1:
                $matching_percentage = $bonus_config->level1;
                break;
            case 2:
                $matching_percentage = $bonus_config->level2;
                break;
            case 3:
                $matching_percentage = $bonus_config->level3;
                break;
            case 4:
                $matching_percentage = $bonus_config->level4;
                break;
            case 5:
                $matching_percentage = $bonus_config->level5;
                break;
        }
        
        if ($matching_percentage && $matching_percentage != 'NULL') {
            return $downline_bonus * ($matching_percentage / 100);
        }
        
        return 0;
    }
    public function getDownlineTree($user_id, $level = 1, $max_level = 8)
    {
        if ($level > $max_level) {
            return null;
        }
        
        $this->db->select('id, first_name, last_name, left_node, right_node');
        $this->db->where('id', $user_id);
        $user = $this->db->get('users')->row();
        ...
        if ($user->left_node) {
            $tree['left'] = $this->getDownlineTree($user->left_node, $level + 1, $max_level);
        }
        
        if ($user->right_node) {
            $tree['right'] = $this->getDownlineTree($user->right_node, $level + 1, $max_level);
        }
        
        return $tree;
    }
    // end  new function for calculate  Rack  ==================================               
}
