...
class Sales_model extends CI_Model
{
    ...
    {  
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            if ($stockmoves) {
                foreach ($stockmoves as $stockmove) {
                    if ($stockmove['product_type'] != 'combo') {
                        ...
                        if($data['saleman_by']){
                            $stockmove['saleman_by']  = $data['saleman_by'];
                        }
            ...
            //-------update bonus bv----------
            $saleman_by = $this->site->getUser($data['saleman_by']);
            if ($data['saleman_by']) {
              $this->syn_bonus_sponsor($sale_id,$items,$data['saleman_by']);
            }
            //---------close update bv------
    ...
    {  
        ...
        if ($this->db->update('sales', $data, ['id' => $id]) && $this->db->delete('sale_items', ['sale_id' => $id]) && $this->db->delete('sale_combo_items', ['sale_id' => $id]) && $this->db->delete('costing', ['sale_id' => $id])) {
            ...
            //-------update bonus bv----------
            if ($data['saleman_by']) {
                //  $this->site->update_bv_qty($data['grand_total'],$data['saleman_by']);
              $this->syn_bonus_sponsor($id,$items,$data['saleman_by']);
            }
            //---------close update bv------
            $this->db->update('users', ['commission_product' => ($staff->commission_product + $commission_product)], ['id' => $data['saleman_by']]);
    ...
    public function deleteSale($id)
    {
        ...
        if ($this->db->delete('sale_items', ['sale_id' => $id]) && $this->db->delete('sales', ['id' => $id]) && $this->db->delete('costing', ['sale_id' => $id])) {
            ...
            //----------delete bonus--------
            $this->db->delete('sponsor_member', ['sale_id' => $id]);
    ...
    public function getAllEmployee() {
        $this->db->select(
            $this->db->dbprefix('users').".id as id, " . $this->db->dbprefix('users') . ".emp_code, 
            CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ' ," . $this->db->dbprefix('users') . ".last_name) AS fullname, 
            " . $this->db->dbprefix('users') . ".gender, nationality, position, employeed_date, phone, company, active");
        $this->db->from("users");
    ...
    // public function getSaleBVBySalemanID($id,$submembers = [])
    // {
    //     // $ids = array_column($submembers, 'id');
    //     // $salemans_id = json_encode($ids,JSON_NUMERIC_CHECK);
    //     if($submembers){
    //         $ids = array_column($submembers, 'id');
    ...
    //                 (Select max_serial from {$this->db->dbprefix('products')} where id ={$this->db->dbprefix('sale_items')}.product_id)
    //             ) as total_bv,

    //         ");
    //         $this->db->join('sale_items', 'sales.id=sale_items.sale_id', 'left');
    //         // $this->db->where("sales.saleman_by IN (" . implode(',', $ids) . ")");
    //          $this->db->where_in('sales.saleman_by', $ids);
    ...
    public function getSaleBVBySalemanID_($id, $submembers = [])
    {
        if (empty($submembers)) {
            return false;
        }
        ...
        foreach ($submembers as $m) {
            if (isset($m->left_node)) {
                $leftIds[] = (int) $m->id;
            }
            if (isset($m->right_node)) {
                $rightIds[] = (int) $m->id;
            }
        }
        ...
        $leftIdsStr  = implode(',', $leftIds ?: [0]);
        $rightIdsStr = implode(',', $rightIds ?: [0]);
        ...
        $this->db->select("
            ...
            (
                ...
                (
                    ...
                    WHERE bpas_products.id = bpas_sale_items.product_id
                )
            ) AS total_bv,
            CASE
                WHEN bpas_sales.saleman_by IN ($leftIdsStr) THEN 'left'
                WHEN bpas_sales.saleman_by IN ($rightIdsStr) THEN 'right'
            END AS side
        ", false);

        $this->db->from('sales');
        $this->db->join('sale_items', 'sales.id = sale_items.sale_id', 'left');
        $this->db->where_in('sales.saleman_by', array_merge($leftIds, $rightIds));

        $q = $this->db->get();

        return ($q->num_rows() > 0) ? $q->result() : false;
    }
    public function getSumPVBySaleman($saleman_id, $start_date=null,$end_date=null)
    {
        $this->db->select('SUM(pv) AS tpv');
        $this->db->from('sales');
        $this->db->where('saleman_by', $saleman_id);
        // if($start_date){
    ...
    public function getSumBV($saleman_id, $product_type = 'combo', $start_date = null, $end_date = null)
    {
        if (!$saleman_id) {
            return (object)['total_bv' => 0];
        }
        ...
        $row = $this->db->get()->row();
        return $row ?: (object)['total_bv' => 0];
    }
    public function getSumPV($user_id, $start_date = null, $end_date = null)
    {
        if (!$user_id) {
            return 0;
        }
        ...
        $row = $this->db->get()->row();
        return $row->total_pv ?? 0;
    }
    public function getSalesWithSponsorBonus($user_id, $start_date = null, $end_date = null)
    {
        $this->db->select('
            si.*,
        ...
        $this->db->join('users u', 'u.id = si.sponsor_id', 'left');
        $this->db->where('si.saleman_by', $user_id);
        
        if ($start_date && $end_date) {
            $this->db->where('si.sale_date >=', $start_date);
            $this->db->where('si.sale_date <=', $end_date);
        }
    ...
    public function getTotalSponsorBonus($user_id, $start_date = null, $end_date = null)
    {
        $this->db->select("SUM(sponsor_bonus) AS total_bonus");
        $this->db->from('sale_items');
    ...
    public function getBVBreakdown($user_id, $start_date = null, $end_date = null)
    {
        $this->db->select("
            product_type,
            SUM(max_serial) as total_bv,
        ...
        foreach ($query->result() as $row) {
            $result[$row->product_type] = $row->total_bv;
            $result['details'][] = $row;
        }
        
        return (object)$result;
    }
    public function getWeeklySales($user_id, $start_date, $end_date)
    {
        $this->db->select('
            DATE(sale_date) as sale_day,
            SUM(CASE WHEN product_type = "combo" THEN max_serial ELSE 0 END) as combo_bv,
            SUM(CASE WHEN product_type = "standard" THEN max_serial ELSE 0 END) as standard_bv,
            SUM(max_serial) as total_bv,
    ...
    public function getMonthlySales($user_id, $year)
    {
        $this->db->select('
    ...
    public function getSaleBVBySalemanID($id, $submembers = [], $start_date=null,$end_date=null)
    {
        if (empty($submembers)) return false;

        $leftIds  = [];
        $rightIds = [];
        foreach ($submembers as $m) {
            if (isset($m->left_node))  $leftIds[]  = (int)$m->id;
            if (isset($m->right_node)) $rightIds[] = (int)$m->id;
        }
        if (empty($leftIds) && empty($rightIds)) return false;
        ...
        $this->db->select("
            ...
            us.id as user_id,
            -- total_bv calculation
            CASE
                WHEN EXISTS (
                    SELECT 1
                    FROM bpas_sale_combo_items sc_check
                    WHERE sc_check.sale_item_id = si.id
                )
                THEN (
                    SELECT SUM(sc2.quantity * p2.max_serial)
                    FROM bpas_sale_combo_items sc2
                    LEFT JOIN bpas_products p2 ON p2.id = sc2.product_id
                    WHERE sc2.sale_item_id = si.id
                )
                ELSE si.quantity * p.max_serial
            END AS total_bv,
            CASE
                WHEN s.saleman_by IN ($leftIdsStr) THEN 'left'
                WHEN s.saleman_by IN ($rightIdsStr) THEN 'right'
            END AS side
        ", false);

        $this->db->from('bpas_sales s');
        $this->db->join('bpas_sale_items si', 's.id = si.sale_id', 'left');
        $this->db->join('users us', 'us.id = s.saleman_by', 'left');
        $this->db->join('bpas_products p', 'p.id = si.product_id', 'left');


        if($start_date){
              $this->db->where('s.date BETWEEN "' . $start_date . '" and "' . $end_date . '"');
        }

        $this->db->where_in('s.saleman_by', array_merge($leftIds, $rightIds));
     ...
     public function getSaleBVBySalemanIDALL($id, $submembers = [])
    {
        if (empty($submembers)) return false;
    ...
    //----------update bonus-----------
    public function syn_bonus_sponsor($sale_id,$items=[], $saleman_id)
    {    
       
        $mainsponsor  = $this->auth_model->getSponsorByID($saleman_id);
        if($mainsponsor->sponsor_id){
            if($sale_id){
                $this->db->delete('sponsor_member', ['sale_id' => $sale_id]);
            }
            foreach($items as $item){
                if($item['product_code'] =='PK01'){
                    $bonus = 20;
                }elseif($item['product_code'] =='PK02'){
                    $bonus = 40;
                }else {
                    $bonus = 100;
                }
                $data = [
                    ...
                    'sponsor_id'    => $mainsponsor->sponsor_id,
                    'bonus'         => $bonus
                ];
                $this->db->insert('sponsor_member', $data);
     ...
     public function getTotalBVByLeftSaleman($id, $submembers = [])
    {
        if (empty($submembers)) return false;
     ...
     //   $rightIdsStr = implode(',', $rightIds ?: [0]);
        ...
        $this->db->where_in('s.saleman_by', array_merge($leftIds));
    ...
    public function getSumBVBySalemanID($id, $submembers = [])
    {
        if (empty($submembers)) return false;
        ...
        $this->db->select("
            sum(CASE
                WHEN EXISTS (
                    SELECT 1
                    FROM bpas_sale_items sc_check
                    WHERE sc_check.sale_item_id = si.id
                )
                THEN (
                    SELECT SUM(sc2.quantity * p2.max_serial)
                    FROM bpas_sale_items sc2
                    LEFT JOIN bpas_products p2 ON p2.id = sc2.product_id
                    WHERE sc2.sale_item_id = si.id
                )
                ELSE si.quantity * p.max_serial
