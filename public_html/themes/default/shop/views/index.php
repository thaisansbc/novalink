                                                ...
                                                <div class="details" style="transition: all 100ms ease-out 0s;">
                                                    ...
                                                    <div class="stats-container">
                                                        ...
                                                        <span class="product_price">
                                                            <?php
                                                            if ($fp->promotion) {
                                                                echo '<del class="text-red">' . $this->bpas->convertMoney(isset($fp->special_price) && !empty(isset($fp->special_price)) ? $fp->special_price : $fp->price) . '</del><br>';
                                                                echo $this->bpas->convertMoney($fp->promo_price);
                                                            } else {
                                                                echo $this->bpas->convertMoney(isset($fp->special_price) && !empty(isset($fp->special_price)) ? $fp->special_price : $fp->price);
                                                            } ?>
                                                        </span>
                                                        <?php
                                                    } ?>
...
<section class="page-contents hide">
    <div class="container">
        <div class="row">
            <?php 
            ...
            ?>
            <div class="col-xs-3">
                <a href="<?= base_url('post/').$post->slug;?>" title="JC Scheidell" class="vc_gitem-link vc-zone-link">
                    <div>
                        <img src="<?= base_url('assets/uploads/').$post->image;?>" class="vc_gitem-zone-img" style="max-width: 250px;">
                    </div>
                </a>
                <h4 style="text-align: left"><?= $post->title;?></h4>
                <?php 
                if(strlen($post->detail) > 250){
                ?>
                <div>
                    <?= substr($post->detail, 0, 250);?> ...
                </div>
                <?php
                    echo '<a href="'.base_url('post/').$post->slug.'" class="button btn btn-primary">Read more</a>';
                }else{
                    echo $post->detail;
                }
                ?>
