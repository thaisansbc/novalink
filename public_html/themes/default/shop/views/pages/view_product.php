                        ...
                        <div class="panel panel-default margin-top-lg">
                            ...
                            <div class="panel-body mprint">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <p style="font-size:1.1em;font-weight:bold;"><?= $product->name; ?></p>
                                        <?= $product->cf3; ?>
                                        <div>
                                            <strong>តម្លៃ / <?= lang('price'); ?>:
                                            <?= $this->bpas->convertMoney(isset($product->special_price) ? $product->special_price : $product->price); ?>
                                            </strong>
                                        </div>
                                        <br>
                                        <div class="details">
                                            <?= $this->bpas->decode_html($product->product_details); ?>
                                        </div>
                                        ...
                                        <div class="table-responsive hide">
                                            <table class="table table-bordered table-striped dfTable table-right-left">
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td><?= lang('price'); ?></td>
                                                        <td><?= $this->bpas->convertMoney(isset($product->special_price) ? $product->special_price : $product->price); ?></td>
                                                    </tr>
                                                    <?php
                                                    } ?>
                                                    ...
                                                    if ($product->promotion) {
                                                        echo '<tr><td>' . lang('promotion') . '</td><td><strong>' . $this->bpas->convertMoney($product->promo_price) . '</strong><br>' . ($product->start_date && $product->start_date != '0000-00-00' ? lang('start_date') . ': <strong>' . $this->bpas->hrsd($product->start_date) . '</strong><br>' : '') . ($product->end_date && $product->end_date != '0000-00-00' ? lang('end_date') . ': <strong>' . $this->bpas->hrsd($product->end_date) . '</strong>' : '') . '</td></tr>';
                                                    }
                                                    ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <!-- <td><?= $this->bpas->formatQuantity($warehouse->quantity); ?></td> -->
                                                    </tr>
                                                    <?php
                                                    } ?>
                                                    ...
                                                    <?php if (!empty($options)) {
                                                        foreach ($options as $option) {
                                                            if ($option->wh_qty != 0) {
                                                                ...
                                                                // echo '<tr><td>' . lang('in_stock') . ': ' . $this->bpas->formatQuantity($option->wh_qty) . '</td>';
                                                                if (!$shop_settings->hide_price) {
                                                                    echo '<tr><td>' . lang('in_stock') . ': ' . ($option->wh_qty ? lang('yes') : lang('no')) . '</td>';
                                                                    echo '<td>' . lang('price') . ': ' . $this->bpas->convertMoney((isset($product->special_price) ? $product->special_price : $product->price) + $option->price) . '</td>';
                                                                    echo '</tr>';
                                                                } else {
                                        ...
                                        <div class="form-group">
                                            ...
                                            if ($variants) {
                                                foreach ($variants as $variant) {
                                                    $opts[$variant->id] = $variant->name . ($variant->price > 0 ? ' (+' . $this->bpas->convertMoney($variant->price, true, false) . ')' : ($variant->price == 0 ? '' : ' (+' . $this->bpas->convertMoney($variant->price, true, false) . ')'));
                                                }
                                                echo form_dropdown('option', $opts, '', 'class="form-control selectpicker mobile-device" required="required"');
                                            } ?>
                                        </div>
                                        <div class="box_addcart" style="width: 250px;">
                                            <div class="form-group">
                                ...
                                <ul class="nav nav-tabs" role="tablist">
                                    <li class="active"><a href="#points" role="tab" data-toggle="tab"><strong>មុខងារចម្បង / Key points</strong></a></li>
                                    <li><a href="#benefit" role="tab" data-toggle="tab"><strong>អត្ថប្រយោជន៏/ Benefit</strong></a></li>
                                    <li><a href="#users" role="tab" data-toggle="tab"><strong>អ្នកប្រើប្រាស់ដែលណែនាំ​ / Recommended Users</strong></a></li>
                                    <li><a href="#usage" role="tab" data-toggle="tab"><strong>របៀបប្រើប្រាស់/ Recommended Usage</strong></a></li>
                                </ul>
                                <div class="tab-content padding-lg white bordered-light" style="margin-top:-1px;">
                                    <div role="tabpanel" class="tab-pane fade in active" id="points">
                                         <?= $this->bpas->decode_html($product->details); ?>
                                    </div>
                                    <div role="tabpanel" class="tab-pane fade" id="benefit">
                                        <?= $this->bpas->decode_html($product->cf4); ?>
                                    </div>
                                    <div role="tabpanel" class="tab-pane fade" id="users">
                                        <?= $this->bpas->decode_html($product->cf5); ?>
                                    </div>
                                    <div role="tabpanel" class="tab-pane fade" id="usage">
                                        <?= $this->bpas->decode_html($product->cf6); ?>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="details" style="transition: all 100ms ease-out 0s;">
                                ...
                                <div class="stats-container">
                                    ...
                                    <span class="product_price">
                                        <?php
                                        if ($fp->promotion) {
                                            echo '<del class="text-red">' . $this->bpas->convertMoney(isset($fp->special_price) && !empty($fp->special_price) ? $fp->special_price : $fp->price) . '</del><br>';
                                            echo $this->bpas->convertMoney($fp->promo_price);
                                        } else {
                                            echo $this->bpas->convertMoney(isset($fp->special_price) && !empty($fp->special_price) ? $fp->special_price : $fp->price);
                                        } ?>
                                    </span>
                                    <?php
                                } ?>
