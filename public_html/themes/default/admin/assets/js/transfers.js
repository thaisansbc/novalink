...
$(document).on('click', '#editItem', function () {
    ...
    var unit = $('#punit').val();
    var unit_name = $('#punit option:selected').text();
    ...
    toitems[item_id].row.unit_name = unit_name,
...
function loadItems() {
    if (localStorage.getItem('toitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id, item_type = item.row.type, item_cost = item.row.cost, item_qty = item.row.qty, item_bqty = item.row.quantity_balance, item_oqty = item.row.ordered_quantity, 
                item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, 
                item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            var unit_cost  = item.row.real_unit_cost;
            var product_unit = item.row.unit, product_unit_name = item.row.unit_name, base_quantity = item.row.base_quantity;
            var pr_tax = item.tax_rate;
            var pr_tax_val = 0, pr_tax_rate = 0;
            var item_expiry = item.expiry;
            ...
            tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><span class="sname" id="name_' + row_no + '">' + item_code +' - '+ item_name + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + ' (' + product_unit_name + ')' + '</span> <i class="pull-right fa fa-edit tip tointer edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';
            if (site.settings.product_expiry == 1) {
                tr_html += '<td><input  style="' + ((item_expiry != null && item_expiry != '') ? '' : 'display: none;') + '" class="form-control rexpiry" name="expiry[]" type="text" value="' + (item_expiry ? fsd(item_expiry) : null) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="expiry_' + row_no + '" readonly></td>';
            }
            tr_html += 
                '<td class="text-right show-cost">' + 
                    ...
                    '<input class="realucost" name="real_unit_cost[]" type="hidden" value="' + item.row.real_unit_cost + '"><span class="text-right scost" id="scost_' + row_no + '">' + formatMoney(item_cost) + '</span>' + 
                '</td>';
            ...
            if (site.settings.tax1 == 1) {
                tr_html += '<td class="text-right show-cost"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + pr_tax.id + '"><span class="text-right sproduct_tax" id="sproduct_tax_' + row_no + '">' + (pr_tax_rate ? '(' + formatDecimal(pr_tax_rate) + ')' : '') + ' ' + formatMoney(pr_tax_val * item_qty) + '</span></td>';
            }
            tr_html += '<td class="text-right show-cost"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_cost) - item_discount + parseFloat(pr_tax_val)) * parseFloat(item_qty))) + '</span></td>';
            ...
            if (item.options !== false) {
                ...
                var new_arr = {};
                if (toitems != null) {
                    $.each(toitems, function(index, obj) {
                        if (obj.row.id == product_id && obj.expiry == item_expiry) {
                            ...
                            new_arr[obj.row.id + (obj.expiry != null ? ('_' + obj.expiry) : '')] += parseFloat(obj.row.base_quantity);
                        }
                    });
                }
                if (item.pitems) {
                    $.each(item.pitems, function () {
                        if (Object.keys(new_arr) == (this.product_id + (this.expiry != null ? ('_' + this.expiry) : ''))) {
                            if (parseFloat(Object.values(new_arr)) > parseFloat(this.quantity_balance)) {
                                $('#row_' + row_no).addClass('danger');
        ...
        var col = 2;
        if ((!Admin && !Owner) && show_cost == 0) { col = 1; }
        ...
        if (site.settings.tax1 == 1) {
            tfoot += '<th class="text-right show-cost">'+formatMoney(product_tax)+'</th>';
        }
        tfoot += '<th class="text-right show-cost">'+formatMoney(total)+'</th><th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></th></tr>';
