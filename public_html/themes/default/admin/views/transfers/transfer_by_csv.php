...
<script type="text/javascript">
    ...
    ?>
        ...
        //-----------biller-------------
        var to_biller;
        $('#to_biller').on("select2-focus", function (e) {
            to_biller = $(this).val();
        }).on("select2-close", function (e) {
            if ($(this).val() != '' && $(this).val() == $('#from_biller').val()) {
                $(this).select2('val', to_biller);
                bootbox.alert('<?= lang('please_select_different_biller') ?>');
            }
        });
        var from_biller;
        $('#from_biller').on("select2-focus", function (e) {
            from_biller = $(this).val();
        }).on("select2-close", function (e) {
            if ($(this).val() != '' && $(this).val() == $('#to_biller').val()) {
                $(this).select2('val', from_biller);
                bootbox.alert('<?= lang('please_select_different_biller') ?>');
            }
        });
    });
                                ...
                                <?php if ($Owner || $Admin || !$this->session->userdata('biller_id')) { ?>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            echo form_dropdown('biller', $bl, (isset($_POST['biller']) ? $_POST['biller'] : $Settings->default_biller), 'id="from_biller" data-placeholder="' . lang("select") . ' ' . lang("biller") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                                            ?>
                                        </div>
                                    </div>
                                <?php } else {
