...
<script>
    $(document).ready(function() {
        oTable = $('#SLData').dataTable({
            ...
            'bProcessing': true,
            'bServerSide': true,
            'sAjaxSource': '<?= admin_url('sales/getSales' . ($biller_id ? '/' . $biller_id : '') . '?v=1' . $v . ($this->input->get('shop') ? '&shop=' . $this->input->get('shop') : '') . ($this->input->get('attachment') ? '&attachment=' . $this->input->get('attachment') : '') . ($this->input->get('delivery') ? '&delivery=' . $this->input->get('delivery') : '')); ?>',
            ...
            {
                column_number: 5,
                filter_default_label: "[<?= lang('member'); ?>]",
                filter_type: "text",
                data: []
            },
            {
    ...
    $(function(){
        $('#multi_payment').live('click',function(){
            ...
            if(sale_id==''){
                ...
                var link = '<?= admin_url('sales/add_multi_payment/#######', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#myModal" class="multi_payment"')?>';
